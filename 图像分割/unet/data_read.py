...
trans = transforms.Compose([
    transforms.Resize((256,256)),#修改输送到网络的图像大小
    transforms.ToTensor()])
...
def get_dataloader(batchsize, root1, root2,root3,root4):
    ...
    train_loader = D.DataLoader(MyDataset(img_train, label_train, trans), batch_size=batchsize, shuffle=True,
                                pin_memory=True, num_workers=0)
    valid_loader = D.DataLoader(MyDataset(img_test, label_test, trans), batch_size=batchsize,
                                pin_memory=True, num_workers=0)
