...
def read_datesets(root,rat):
    ...
    for i1 in list_read:
        ...
        # 按比例随机采样验证样本
        val = random.sample(images, k=int(len(images) *rat))
...
def get_data(root,batchsize,rat):
    train_path, train_label, val_path, val_label = read_datesets(root,rat)  # 获取4个列表
    ...
    train_loader = Data.DataLoader(train_data_set, batch_size=batchsize, shuffle=True, num_workers=0)  # 导入训练集
    val_loader = Data.DataLoader(val_data_set, batch_size=batchsize,  num_workers=0)  # 导入预测集
    return train_loader,val_loader,val_path
