...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=16, in_c=3, embed_dim=768):
        super(PatchEmbed,self).__init__()
        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size)
    def forward(self, x):
        # [batchsize,输入通道，图像宽度，图像高度]
        # x.shape [B,3,224,224]
        x = self.proj(x)  # [B,768,14,14]
        x = torch.flatten(x, 2)  # [B,768,196]
        x = torch.transpose(x, 1, 2)  # [B, 196, 768]
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224,patch_size=16, in_c=3, num_classes=3,
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))#分类的token
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches +1, embed_dim))#位置信息
